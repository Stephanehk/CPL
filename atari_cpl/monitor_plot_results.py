...
from stable_baselines3.common import results_plotter,monitor
...
def plot_curves(
    xy_list: List[Tuple[np.ndarray, np.ndarray]],labels, x_axis: str, title: str, figsize: Tuple[int, int] = (8, 2)
) -> None:
    """
    plot the curves
...
def plot_results(
    dirs: List[str],labels, num_timesteps: Optional[int], x_axis: str, task_name: str, figsize: Tuple[int, int] = (8, 2)
) -> None:
    """
    ...
    #     x,y = trial
    #     print (np.mean(y))
    plot_curves(xy_list,labels, x_axis, task_name, figsize)


timesteps = 10000000

log_dir1 = "PhoenixNoFrameskip-v4_monitor_files"
#
# exp_name2= "batch_size=16_seed=1_env=Walker2d-v0_baseline_n_prefs_per_round=100_n_rounds=30_max_queue_size=None_teacher_temp=0"
# log_dir2 = exp_name2 + "/monitor/"


labels = [""]
plot_results([log_dir1],labels, timesteps, results_plotter.X_TIMESTEPS, "TITLE")
plt.show()
